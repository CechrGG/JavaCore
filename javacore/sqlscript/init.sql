CREATE DATABASE IF NOT EXISTS `com_petapp`;
USE `com_petapp`;
CREATE TABLE IF NOT EXISTS `auth_cat`(
...
`gmt_birthday` DATE NOT NULL DEFAULT '1970-1-1' COMMENT '生日',
`gmt_deathday` DATE NOT NULL DEFAULT '9999-9-9' COMMENT '忌日',
...
CREATE TABLE IF NOT EXISTS `auth_slave`(
...
`gmt_birthday` DATE NOT NULL DEFAULT '1970-1-1' COMMENT '生日',
`gmt_enslaved` DATE NOT NULL DEFAULT '9999-12-31' COMMENT '被奴役时间',
`memo` VARCHAR(255) NOT NULL DEFAULT '无' COMMENT '备注',
`gmt_create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '加入时间',
`gmt_update` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间'
...
`code` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '表格代码',
`name` VARCHAR(64) NOT NULL COMMENT '表格名',
`tbname` VARCHAR(30) NOT NULL COMMENT '数据表名',
`keycode` VARCHAR(20) NOT NULL COMMENT '关键字'
)ENGINE=INNODB DEFAULT CHARSET=utf8;